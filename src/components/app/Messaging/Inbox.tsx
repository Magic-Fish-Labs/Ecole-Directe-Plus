...
export default function Inbox({ selectedFolder }) {
	...
	const contentLoadersRandomValues = useRef({
		authorWidth: Array.from(
			{ length: 13 },
			(_) => Math.round(Math.random() * 100) + 100,
		),
		subjectWidth: Array.from(
			{ length: 13 },
			(_) => Math.floor(Math.random() * 150) + 150,
		),
		dateWidth: Array.from(
			{ length: 13 },
			(_) => Math.floor(Math.random() * 50) + 50,
		),
		containsFiles: Array.from({ length: 13 }, (_) => Math.random() > 0.6),
	});
	...
	return (
		<div id="inbox">
			...
			messageFolders?.find((folder) => folder.id === selectedFolder)
				?.fetched ? (
				messages.filter(
					(message) => message.folderId === selectedFolder,
				).length > 0 ? (
					<ScrollShadedDiv className="messages-container">
						<ul>
							{messages
								.filter(
									(message) =>
										message.folderId === selectedFolder,
								)
									...
									>
										<h4 className="message-subject">
											...
											<span className="actions">
												...
												>
													<MarkAsUnread className="mark-as-unread-icon" />
												</button>{" "}
					...
					<p className="no-message-received">
						Ce dossier est vide. Peut-être qu'il attend juste un
						miracle... ou un clic
					</p>
				) : (
					<p className="no-message-received">
						Vous n'avez reçu aucun message. Tendez l'oreille et
						profitez de cet instant de silence
					</p>
				)
			) : (
				<ScrollShadedDiv className="messages-container">
							...
							>
								<h4 className="message-subject">
									<span className="author-name">
										<ContentLoader
											...
											style={{
												width: `min(${contentLoadersRandomValues.current.authorWidth[index]}px, 100%)`,
												maxHeight: "20px",
											}}
										>
											<rect
									...
									<span className="actions">
										{contentLoadersRandomValues.current
											.containsFiles[index] && (
											<AttachmentIcon className="attachment-icon" />
										)}
									</span>
								...
								<p className="message-author">
									<ContentLoader
										...
										style={{
											width: `min(${contentLoadersRandomValues.current.subjectWidth[index]}px, 60%)`,
											maxHeight: "16px",
										}}
									>
										<rect
								...
								<p className="message-date">
									<ContentLoader
										...
										style={{
											width: `min(${contentLoadersRandomValues.current.dateWidth[index]}px, 30%)`,
											maxHeight: "16px",
										}}
									>
										<rect
