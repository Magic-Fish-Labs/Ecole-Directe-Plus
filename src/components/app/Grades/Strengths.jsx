        ...
        <WindowContent>
            {grades && grades[activePeriod]
                ? <ol className={"strengths-container " + (displayType === 0 ? "display-strengths" : "display-weaknesses")}>
                    {
                        (displayType === 0 ? strengths : weaknesses).map((strength, idx) => <li key={strength?.subject?.name || crypto.randomUUID()} onClick={() => setSelectedGradeElement(strength?.subject)} style={{ "--order": idx }} className="strength-container">
                            <span className="strength-wrapper">
                ...
                : <ol className="strengths-container display-strengths">
                    {
                        Array.from({ length: 3 }, () => <li key={crypto.randomUUID()} style={{ "--order": -69 /* skip the animation */ }} className="strength-container">
